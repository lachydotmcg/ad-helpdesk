...
- **Per-tenant credential storage.** Each AID Helpdesk tenant supplies its own Entra app registration (tenant ID, client ID, client secret), stored per-tenant rather than as a shared environment variable. One AID Helpdesk deployment serving multiple organisations never lets one tenant's Graph calls run under another tenant's credentials.
- **Secrets encrypted at rest.** The Entra client secret and the tenant SMTP password are encrypted in the database with Fernet (`cloud/secrets_crypto.py`) when `SETTINGS_ENCRYPTION_KEY` is configured. Encryption is transparent to callers: values are decrypted only when read for use and are never echoed back through the settings API (the API redacts them to a blank placeholder). Without the key set the app falls back to plaintext storage and logs a startup warning, so set the key before real credentials are entered. Secrets are never logged, and a value that cannot be decrypted (rotated or mismatched key) returns empty rather than leaking ciphertext.
