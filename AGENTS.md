...
- AI assistant chat (`/dashboard/chat`) — Codex Haiku with tool-use style prompting
...
- `WRITE` — reversible mutations, the AI assistant can auto-resolve if tenant enables it
- `DESTRUCTIVE` — always requires human confirmation, AI auto-resolve is hard-blocked at Python layer regardless of prompt
...
### WRITE (reversible, can be auto-resolved by the AI assistant if enabled)
...
The AI assistant is embedded in the dashboard. Key behaviours:
- Uses Codex Haiku via Anthropic API
- System prompt includes: all available actions + args, tenant's AI context (custom free-text), list of enabled custom scripts
...
- **Bulk/conditional ops:** the AI assistant can issue `bulk_move_users` with a full rules array in a single call — no looping. Example: move 500 students based on username suffix patterns, with auto-OU creation.
- **Custom scripts:** the AI assistant resolves a script slug → fetches `ps_content` from DB → embeds as `args[0]` before queuing to agent. The agent never deals with slugs, only PS content.
- **Auto-resolve:** the AI assistant can auto-close tickets if tenant setting `auto_resolve` is on, restricted to actions in `auto_actions` list.

### AI Config Page
Three panels:
1. **AI Behaviour** — configurable name, auto-resolve toggle, enabled auto-actions, ai_context free-text
2. **Scheduled Reports** — enable/disable, frequency (daily/weekly/monthly), day, hour, recipient emails
3. **Custom Scripts** — upload `.ps1` files, set slug/name/description/args/classification, the AI assistant can call by slug
...
- [x] AI assistant chat with action execution
- [x] Configure AI page (AI Behaviour + Scheduled Reports + Custom Scripts panels)
...
- [x] Custom PowerShell scripts (upload, the AI assistant callable by slug)
...
- [x] AI ticket auto-resolve

**NOT YET DONE:**
- [ ] Stripe billing (explicitly deferred — build last)
- [ ] AI persistent memory / learning system (see Future Vision below)
...
- `cloud/app.py` — TIME_SAVED_MINUTES, `/dashboard/api/time-saved`, custom scripts CRUD endpoints, AI custom-script slug resolution, scheduled reports system, ai_context/report settings, upgraded system prompt (bulk ops, conditional logic, custom scripts)
- `cloud/db.py` — `custom_scripts` table (v5 migration), full CRUD functions, `get_action_counts_this_month`, `list_all_tenants`
- `cloud/templates/dashboard.html` — time saved card, Configure AI page, custom scripts modal, ticket minimise button, draggable mini-bar, `loadAiConfig()`, `loadScriptsList()`, `minimiseTicket()` etc.

**Suggested commit message:**
```
feat: custom scripts, bulk conditional AD ops, Configure AI page, time saved tally, scheduled reports, ticket mini-bar

- Custom PS scripts: upload via UI, callable by the AI assistant via slug, classification per script
...
- Configure AI page: Behaviour panel, Scheduled Reports panel, Custom Scripts panel
- AI system prompt: bulk/conditional op rules, custom scripts context, ai_context injection
...
## Future Vision — AI Memory / Persona System

**This was discussed but not built yet. High priority next feature after Stripe.**

The idea: move the AI assistant from a stateless-per-request AI to a persistent agent that learns about the organisation over time.
...
- Before each AI response, relevant memories are retrieved (keyword match at small scale, vector similarity at large scale) and injected into the system prompt
- Tenants can view/edit/delete memories from the Configure AI page
...
| `ANTHROPIC_API_KEY` | Codex Haiku for the AI assistant |
...
3. **Slug resolution happens in the cloud, not the agent.** When the AI assistant calls `run_custom_script` with a slug, `app.py` fetches the PS content and embeds it as `args[0]` before queuing. The agent receives PS content directly.
4. **Bulk ops use a single PowerShell call.** `bulk_move_users` takes all rules as a JSON array, builds a single PS script. The AI assistant should never loop `move_user` for batch operations.
